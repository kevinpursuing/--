             ...
             <div class="row">
                ...
                <tbody>
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$l): $mod = ($i % 2 );++$i;?><tr class="">
							<td><?php echo ($i); ?></td>
							<td><?php echo ($l["id"]); ?></td>
							<td><?php echo ($l["adm_name"]); ?></td>
							<td><?php echo ($l["gname"]); ?></td>
							<td><?php echo (date('Y-m-d H:i:s',$l["ltime"])); ?></td>
							<td>
								<a class="tan_edit" data-toggle="modal" href="#bianjibox<?php echo ($l["id"]); ?>">编辑</a>
								|
								<?php if($l['state']==1){ ?>
									<a href="javascript:;" onclick="del_admin(<?php echo ($l["id"]); ?>,'admin',this,'id')">删除</a>
								<?php }else{ ?>
									<a href="javascript:;" onclick="hui_admin(<?php echo ($l["id"]); ?>,'admin',this,'id')">恢复</a>
									|
									<a href="javascript:;" class="cddel_admin_btn" onclick="cddel_admin(<?php echo ($l["id"]); ?>,'admin',this,'id')">彻底删除</a>
								<?php } ?>
							</td>
							<!--编辑框 start-->
								...
								<div class="modal-dialog">
									<div class="modal-content">
										...
										<div class="modal-body ">
											<form action="<?php echo U('User/admins_edit');?>" method="post">
												<input type="hidden" name="id" value="<?php echo ($l["id"]); ?>">
												<div class="form-group">
													<label>用户名</label>
													<input id="name" name="adm_name" value="<?php echo ($l["adm_name"]); ?>" class="form-control">
												</div>
												<div class="form-group">
													<label>密码</label>
													<input id="title" type="password" name="pwd" value="" class="form-control">
												</div>
												<div class="form-group">
													<label>分组</label>
													<select name="gid" class="form-control">
														<?php if(is_array($groups)): $i = 0; $__LIST__ = $groups;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$g): $mod = ($i % 2 );++$i;?><option value="<?php echo ($g["id"]); ?>"  <?php if($g['id']==$l['gid']){ ?>selected="selected"<?php } ?>><?php echo ($g["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
													</select>
												</div>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<form role="form" action="<?php echo U('User/admins_edit');?>" method="post" >
							<input type="hidden" name="id" value="">
							<div class="form-group">
								<label>用户名</label>
								<input name="adm_name" value="" class="form-control">
							</div>
							<div class="form-group">
								<label>密码</label>
							...
							<div class="form-group">
								<label>分组</label>
								<select name="gid" class="form-control">
									<?php if(is_array($groups)): $i = 0; $__LIST__ = $groups;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$g): $mod = ($i % 2 );++$i;?><option value="<?php echo ($g["id"]); ?>"><?php echo ($g["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
								</select>
							</div>
							<button type="submit" class="btn btn-primary">保存</button>
						</form>
