...
<div class="wrap">
    <div class="con-layer" style=" padding-top:50px; padding-left:0; padding-right:0;">
		<div class="form-horizontal">
			<input type="hidden" name="bnum" value="<?php echo ($_GET["bnum"]); ?>" />
			<div class="form-group">
				<div class="col-sm-5 inputblwrap ico-user">
					<?php if(empty($user_info["alipay"])): ?><input type="text" class="form-control" name="username" placeholder="请输入支付宝账号" />
					<?php else: ?>
						<input type="text" class="form-control" name="username" readonly value="<?php echo ($user_info["alipay"]); ?>" placeholder="请输入支付宝账号" /><?php endif; ?>
				</div>
			</div>
			
			<div class="form-group">
				<div class="col-sm-5 inputblwrap ico-phone">
					<?php if(empty($user_info["tel"])): ?><input type="number" class="form-control" name="tel" placeholder="联系电话" />
					<?php else: ?>
						<input type="number" class="form-control" name="tel" readonly value="<?php echo ($user_info["tel"]); ?>" placeholder="联系电话" /><?php endif; ?>
				</div>
			</div>
			
			<div class="form-group">
				<div class="col-sm-5 inputblwrap ico-textarea">
					<input type="text" class="form-control beizhuinput" name="realname" value="<?php echo ($user_info["realname"]); ?>" placeholder="支付宝核对姓名" />
				</div>
			</div>
...
function dotobuygo(){
	...
	$.post("<?php echo U('Index/dotobuygo');?>", {bnum:bnum,username:username,tel:tel,realname:realname}, function(v){
		if(v=="duihuancg"){
