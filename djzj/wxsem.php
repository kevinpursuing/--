...
//}
		...
		switch($type) {
			case Wechat::MSGTYPE_TEXT:
				$weObj->text("全民菠菜提示：菠菜妹在查看您的消息")->reply();
				exit;
				break;
...
//		}
		...
		switch($shijian_tui) {
			case Wechat::EVENT_SCAN:
				//$qr_ticket=$weObj->getRevTicket();
				//$qr_ticket_jg=$this->http_get("http://wxtest.esports666.com/weixin.php?a=index&m=index&qrt=".$qr_ticket."");
				//$weObj->text($qr_ticket_jg)->reply();
				$weObj->text("测试我的回复内容")->reply();
				exit;
				break;
			default:
			//getRevSceneId
