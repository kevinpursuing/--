...
class smtp
{
	...
	function sendmail($to,$from, $subject, $content, $T=0){
		...
		$headers[] = "To:=?utf-8?B?".base64_encode($name[0])."?= <$email[0]>";
		...
		$headers[] = ($type == 0) ? "Content-Type: text/plain; charset=utf-8; format=flowed" : "Content-Type: text/html; charset=utf-8; format=flowed";
